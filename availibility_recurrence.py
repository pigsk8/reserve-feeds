            ...
            for week_date in array_week_dates:
                if(week_date.strftime('%A').lower() == day['weekday']):
                    ...
                    if(day['start'] != 0 and day['end'] != 0 and day['start2'] != 0 and day['end2'] != 0):
                        ...
                        timestamp_start = (dt_start - datetime(1970, 1, 1)).total_seconds()

                        dt_end = week_date + timedelta(seconds=(day['end'])*60)
                        timestamp_end = (dt_end - datetime(1970, 1, 1)).total_seconds()
                        
                        dt_start2 = week_date + timedelta(seconds=day['start2']*60)
                        timestamp_start2 = (dt_start2 - datetime(1970, 1, 1)).total_seconds()

                        dt_end2 = week_date + timedelta(seconds=(day['end2']-30)*60)
                        timestamp_end2 = (dt_end2 - datetime(1970, 1, 1)).total_seconds()
                        ...
                        if(day['start'] != 0 and day['end'] != 0):
                            ...
                            dt_end = week_date + timedelta(seconds=(day['end']-30)*60)
                            timestamp_end = (dt_end - datetime(1970, 1, 1)).total_seconds()
                        ...
                        if(day['start2'] != 0 and day['end2'] != 0):
                            ...
                            dt_start = week_date + timedelta(seconds=day['start2']*60)
                            timestamp_start = (dt_start - datetime(1970, 1, 1)).total_seconds()

                            dt_end = week_date + timedelta(seconds=(day['end2']-30)*60)
                            timestamp_end = (dt_end - datetime(1970, 1, 1)).total_seconds()
...
with open('output/availabilityRecurrence'+str(current_day_d)+'.json', 'w') as file:
